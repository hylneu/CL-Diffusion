    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with ((accelerator.accumulate(unet))):
                ...
                loss_aux2 = F.mse_loss(text_pred.float(), target.float(), reduction="mean")

                labels = torch.ones(encoder_hidden_states.size(0)).to(encoder_hidden_states.device)  # Adjust as necessary
                loss_aux3 = cal_cos(encoder_hidden_states, img_state, cos) + contrastive_loss(encoder_hidden_states.float(), img_state.float(), labels)


                # Combined loss
                # Final loss computation
                #clustering_loss = clustering_loss_text + clustering_loss_img
                total_loss = loss + 0.01 * loss_aux2 + 0.001 * loss_aux3  + 0.1 * clustering_loss_img
