...
import open_clip
from cldiffusion import Image_adapter, cal_cos
...
def main(args):
    ...
    img_model, _, preprocess = open_clip.create_model_and_transforms('ViT-H-14', pretrained='stabilityai/stable-diffusion-2-1-base/open_clip_pytorch_model.bin')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                def k_means_clustering_loss(data, centroids):
                    # Debugging: Print shapes
                   # print(f"data shape: {data.shape}, centroids shape: {centroids.shape}")
                    distances = torch.cdist(data, centroids, p=2)  # Compute L2 norm distances
                ...
                # If model predicts variance, discard the prediction and train only on the simplified objective
                if model_pred.shape[1] == 6:
                    model_pred, _ = torch.chunk(model_pred, 2, dim=1)
                ...
                # Flatten or reshape text_pred and img_state
                text_pred_flat = text_pred.view(text_pred.size(0), -1)
                img_state_flat = img_state.view(img_state.size(0), -1)




                # Ensure centroids have the correct dimensions
                num_clusters = 3  # Example number of clusters, can be tuned
                centroid_text = torch.randn(num_clusters, text_pred_flat.shape[1], device=text_pred.device,
                                            requires_grad=True)
                centroid_img = torch.randn(num_clusters, img_state_flat.shape[1], device=img_state.device,
                                           requires_grad=True)

                # # Debugging: Print shapes
                # print(f"text_pred_flat shape: {text_pred_flat.shape}")
                # print(f"img_state_flat shape: {img_state_flat.shape}")
                # print(f"centroid_text shape: {centroid_text.shape}")
                # print(f"centroid_img shape: {centroid_img.shape}")

                # Compute clustering losses
                clustering_loss_text = k_means_clustering_loss(text_pred_flat.float(), centroid_text)
                clustering_loss_img = k_means_clustering_loss(img_state_flat.float(), centroid_img)
                clustering_loss = clustering_loss_text + clustering_loss_img
                ...
                labels = torch.ones(encoder_hidden_states.size(0)).to(encoder_hidden_states.device)  # Adjust as necessary

                # Compute the contrastive loss
                loss_3 = contrastive_loss(encoder_hidden_states.float(), img_state.float(), labels)

                # loss_3 =F.mse_loss(encoder_hidden_states.float(), img_state.float(), reduction="mean")
                # loss = loss + 0.01*loss_aux1 + 0.001*loss_aux2 +0.01*loss_3
                # Combined loss
                # Final loss computation
                clustering_loss = clustering_loss_text + clustering_loss_img
                total_loss = loss + 0.01 * loss_aux1 + 0.001 * loss_aux2 + 0.001 * loss_3 + 0.1 * clustering_loss
            ...
            #     raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")
            #
            # # Compute the primary and auxiliary losses
            # loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
            # loss_aux1 = F.mse_loss(text_pred.float(), target.float(), reduction="mean")
            # loss_aux2 = cal_cos(encoder_hidden_states, img_state, cos)
            # loss_3 = F.mse_loss(encoder_hidden_states.float(), img_state.float(), reduction="mean")
            ...
            # clustering_loss = clustering_loss_text + clustering_loss_img
            #
            # # Combine all components of the loss
            # total_loss = loss + 0.01 * loss_aux1 + 0.001 * loss_aux2 + 0.01 * loss_3 + 0.1 * clustering_loss
            #
            # # Example optimizer setup and backward pass
            # optimizer = torch.optim.Adam([centroid_text, centroid_img] + list(unet.parameters()), lr=1e-3)
            # optimizer.zero_grad()
            # total_loss.backward()
            # optimizer.step()
